...
func Init() {
	...
	post, err := db.Prepare(`CREATE TABLE IF NOT EXISTS posts(id INTEGER PRIMARY KEY AUTOINCREMENT, title	TEXT, content	TEXT, creator_id INTEGER,  created_time	datetime, image	BLOB NOT NULL, count_like INTEGER DEFAULT 0, count_dislike INTEGER DEFAULT 0, category_id INTEGER, FOREIGN KEY(category_id) REFERENCES category(id), FOREIGN KEY(creator_id) REFERENCES users(id) ON DELETE CASCADE ) `)
