...
func Init() {
	...
	voteState, err := db.Prepare(`CREATE TABLE IF NOT EXISTS voteState (id INTEGER PRIMARY KEY AUTOINCREMENT,  user_id INTEGER, comment_id INTEGER, post_id INTEGER, like_state INTEGER  DEFAULT 0, dislike_state INTEGER  DEFAULT 0, FOREIGN KEY(post_id) REFERENCES  posts(id) )`)
